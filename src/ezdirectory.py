...
class DirectoryPlus (str):
    ...
    def __getattribute__(self, __name: str) -> Any:
        ...
        if inspect.isclass(value):
            ...
            # Get the _value_ of the class. If it is not a string, use the class name of the class.
            # 获取到该类的 _value_, 若不是字符串则使用该类的类名.
        ...
        # 如果该值是 DirectoryPlus 类型.
            ...
            # If the value's _value_ attribute is not a string, then its assigned value is itself.
            # 如果该值的 _value_ 属性不是字符串, 那么赋值为它本身.
            if not isinstance(value._value_, str): value._value_ = value

            # Keep the _value_ attribute of the value, maybe it will be useful.
            # 保留该值的 _value_ 属性,也许会有用.
            ...
            if value == target: result = value
                ...
                # Assign _value_ to the new value to avoid calculation errors next time.
                # 将 _value_ 赋给新值, 避免下次计算错误.
